...
export function classifierSoft(features: Features): ClassifierReturn {
  ...
  if (downRatioBTC < 0.2) {
    ...
    if (
      currentPoint.lvl <= -3 &&
      currentPriceNorm > 0.05 &&
      features.targetCoin.downRatio < 0.8
    ) {
      return {
        entry: true,
        probability: 1,
  ...
  if (currentPoint.symbol !== "XLM") {
    if (month >= 11 || month <= 3) {
      ...
      if (
        ...
        currentPriceNorm ${currentPriceNorm.toFixed(2)} >= 0.3 or
      ...
      if (
        ...
        currentPriceNorm > 0.4 &&
        currentBTCPriceNorm > currentPriceNorm
      ) {
        return {
          entry: true,
          probability: 1,
        ...
        reasons.push(`
        SOFT: C.2 rejected currentPoint.lvl ${
          currentPoint.lvl
        } > ${minLevelB} or
        currentBTCPriceNorm ${currentBTCPriceNorm.toFixed(2)} <= 0.4 or
        currentPriceNorm.current ${currentPriceNorm.toFixed(2)} <= 0.4 or
        currentBTCPriceNorm ${currentBTCPriceNorm.toFixed(
          2,
        )} <= currentPriceNorm.current ${currentPriceNorm.toFixed(2)}
        `);
      }
    } else {
      reasons.push(
  ...
  if (
    ...
    if (currentPoint.lvl == -1) {
      if (
        ...
        targetCoin.velocityMove < windowsMs["1h"] * 8 || // coin is falling fast
  ...
  // if (
  //   -Math.ceil(Math.abs(features.market.levelThreshold.meanTop - 3)) >
  //     features.currentPoint.lvl &&
  //   features.currentPoint.lvl >= features.market.levelThreshold.minBottom
  // ) {
  //   return {
  //     entry: true,
  //     probability: 1,
  //     label: `SOFT: O. Level ${currentPoint.lvl} in threshold range`,
  //     reasons: [],
  //   };
  // }
