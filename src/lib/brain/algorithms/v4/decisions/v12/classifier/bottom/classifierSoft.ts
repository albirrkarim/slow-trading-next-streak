...
export function classifierSoft(features: Features): ClassifierReturn {
  ...
  if (downRatioBTC < 0.2) {
    ...
    if (currentPoint.lvl <= -3 && currentPriceNorm.c > 0.05) {
      return {
        entry: true,
        probability: 1,
  ...
  if (currentPoint.lvl <= minLevel && minLevel <= -3) {
    return {
      entry: true,
      probability: 1,
      label:
    ...
    reasons.push(
      `SOFT: rejected B. level ${currentPoint.lvl} > minLevel ${minLevel}`
    );
  }
  ...
  if (currentPoint.symbol !== "XLM") {
    if (month >= 11 || month <= 3) {
      ...
      if (
        ...
        currentPoint.lvl <= (isAnomalyCoin ? -4 : -3) &&
        ...
        reasons.push(`
        SOFT: C.2 rejected currentPoint.lvl ${currentPoint.lvl} > ${
          isAnomalyCoin ? -4 : -3
        } or
  ...
  if (numberOfProfitTrades <= 2) {
    ...
    if (date > 25 && numberOfProfitTrades <= 1) {
      if (currentPoint.lvl <= -2) {
        ...
        reasons.push(`
          SOFT: I. rejected, level ${currentPoint.lvl} > -2
        `);
      }
    } else {
      reasons.push(`
        SOFT: I. rejected enough profit trades this month: ${numberOfProfitTrades} or date ${date} <= 25 or level ${currentPoint.lvl} > -2
      `);
    }
    ...
    if (date > 25 && numberOfProfitTrades <= 1 && downRatioBTC < 0.5) {
      if (currentPoint.lvl <= -1) {
        ...
        reasons.push(`
          SOFT: J. rejected, level ${currentPoint.lvl} > -1
        `);
      }
    } else {
      reasons.push(`
