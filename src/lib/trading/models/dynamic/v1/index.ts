...
/**
 * Passive Trading Strategy V1 - Designed to be dynamic trading multi coins
 * Start development 7 Oktober 2025
 * Updated: 11 Oktober 2025
 *
 * - It use volatility map TO entry
 * - Position size scales with confidence
 * - It can partial selling based on position
 * - Exits on take-profit, stop loss, stop loss plus.
 */
export async function dynamicV1({
  symbol,
  current,
  // fetchKlines,
  config,
  ...
  tradeLog.debug("dynamicV1");
  tradeLog.debug("Symbol ", symbol);
  tradeLog.debug("forceSell ", memory.forceSell);
  tradeLog.debug("memory ", memory.volatility.symbol);
